...
1. **Core Detection System** (`yolo_monitor.py`): Processes camera feed to detect and track objects on shelves
2. **Database Utilities** (`db_utils.py`): Manages product inventory and events
3. **Web Interfaces**:
...
1. **Shelf Setup**: Define shelf regions using the ROI creator
2. **Inventory Initialization**: Initial inventory is detected and saved
3. **Real-time Monitoring**: YOLO continuously monitors the shelves
4. **Event Generation**: When objects are removed or returned, events are recorded
5. **Interfaces Update**: All interfaces update to reflect the current state
6. **Payment Processing**: Cashier system processes payments for removed items
